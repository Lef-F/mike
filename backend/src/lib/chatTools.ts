...
export async function runLLMStream(params: {
    ...
    projectId?: string | null;
    /**
     * Fork addition: MCP servers to expose as additional chat tools. Accepted
     * but not yet wired through to the new upstream tool dispatch — this is a
     * placeholder so callers (projectChat.ts) typecheck. The actual
     * MCP-in-chat integration was lost in the upstream-sync merge and needs
     * to be re-built on top of upstream's runLLMStream tool loop. The MCP
     * routes/management (src/routes/mcp*.ts) still work for configuring
     * servers; only the chat-dispatches-to-MCP path is regressed.
     */
    mcpServers?: import("./mcp/types").LoadedMcpServer[];
